...
export default function Reviews() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentReview((prev) => (prev === config.reviews.length - 1 ? 0 : prev + 1));
    }, 5000);
  ...
  const prevReview = () => {
    setCurrentReview((prev) => (prev === 0 ? config.reviews.length - 1 : prev - 1));
  };
    ...
    <section id="reviews" className="py-20 bg-emerald-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">{config.navigation.reviews}</h2>
          <div className="w-20 h-1 bg-emerald-700 mx-auto mb-6"></div>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Sprawdź, co mówią nasi klienci o naszych usługach. Ich zadowolenie jest dla nas najważniejsze.
          </p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 relative overflow-hidden min-h-[300px] flex flex-col">
            ...
            <div className="relative z-10">
              <div className="text-center mb-8">
                <div className="flex justify-center mb-4">{renderStars(config.reviews[currentReview].rating)}</div>
                <p className="text-lg md:text-xl text-gray-700 leading-relaxed mb-6 italic">
                  &ldquo;{config.reviews[currentReview].text}&rdquo;
                </p>
                <div className="flex items-center justify-center space-x-4">
                  <div>
                    <p className="font-semibold text-gray-900">{config.reviews[currentReview].name}</p>
                    <p className="text-sm text-gray-500">{formatDate(config.reviews[currentReview].date)}</p>
                  </div>
                  ...
                  {config.reviews.map((_, index) => (
                    <button
                      ...
                      className={`w-3 h-3 rounded-full transition-colors ${
                        index === currentReview ? "bg-emerald-700" : "bg-gray-300 hover:bg-emerald-300"
                      }`}
